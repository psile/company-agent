...
def _why(item: dict[str, Any], ctx: dict[str, Any], parts: dict[str, Any]) -> str:
    ...
    if parts.get("project", 0) >= 0.8 and matched:
        return f"你在推进 {project}，这条直接对上了 { ' / '.join(matched[:3]) }。"
    if matched:
        return f"你最近在关注 {matched[0]}，这条正好相关。"
    return f"来自你订阅的 {item.get('source_name')}，值得一读。"
...
def _project_hint(item: dict[str, Any], ctx: dict[str, Any], parts: dict[str, Any]) -> str:
    ...
    if points:
        return f"你目前在 {stage} 阶段，可以参考其中的：{'；'.join(str(x) for x in points[:2])}"
    return f"和你 {stage} 阶段的方向有对照价值。"
