...
- The external quick validator accepts some extra frontmatter keys for compatibility, but EchoBot runtime behavior still only depends on `name`, `description`, and the body text.

## Activation model

- The agent sees a catalog prompt containing each skill `name` and `description`.
- The catalog prompt also tells the agent which skills are already active.
...
- Explicit activations are persisted into history as system messages so later turns can detect the active skill.
...
- `read_skill_resource` expects a relative path inside one of the allowed folders.
...
- If the skill has several reference files, point to the likely first file from the main body.
